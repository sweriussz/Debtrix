...
function calculateMonthlyExpenses() {
    const lifestyleCosts = {
        food: { cheap: 60, basic: 120, premium: 250 },
        housing: { minimal: 150, basic: 300, luxury: 750 },
        transport: { public: 30, basic: 90, premium: 240 }
    };
    
    let total = currentReg.rent * 0.30;  // 30% of rent (region already sets the difficulty)
    total += lifestyleCosts.food[state.lifestyle.food] || 120;
    total += lifestyleCosts.housing[state.lifestyle.housing] || 300;
    total += lifestyleCosts.transport[state.lifestyle.transport] || 90;
...
function getSaveKey(slot) { return `neofinance_save_${slot}`; }

function saveGame(slot) {
    const saveData = {
        state: JSON.parse(JSON.stringify(state)),
        setup: { name: state.name, years: state.years, continent: state.continent },
        timestamp: new Date().toISOString(),
        label: `Year ${state.year} | $${Math.round(state.balance).toLocaleString()} | Tier ${state.tier}`
    };
    localStorage.setItem(getSaveKey(slot), JSON.stringify(saveData));
    SOUNDS.upgrade();
    log(`💾 GAME SAVED to Slot ${slot}`, true);
    renderSaveModal();
}

function loadGame(slot) {
    const raw = localStorage.getItem(getSaveKey(slot));
    if (!raw) { log("⚠️ No save found in that slot."); return; }
    const saveData = JSON.parse(raw);
    // Merge loaded state into current state
    Object.assign(state, saveData.state);
    // Reload region reference
    currentReg = REGIONS[state.continent] || REGIONS.eu;
    SOUNDS.achievement();
    log(`📂 GAME LOADED from Slot ${slot}`, true);
    closeSaveModal();
    refreshUI();
    updateBackground();
}
...
function renderSaveModal() {
    ...
    for (let i = 1; i <= SAVE_SLOTS; i++) {
        ...
        const date = save ? new Date(save.timestamp).toLocaleString() : null;
        container.innerHTML += `
            <div class="save-slot">
                <div class="save-slot-info">
                    <div class="save-slot-num">SLOT ${i}</div>
                    ${save ? `
                        <div class="save-slot-label">${save.label}</div>
                        <div class="save-slot-date">${date}</div>
                    ` : `<div class="save-slot-empty">— EMPTY —</div>`}
                </div>
                <div class="save-slot-actions">
                    <button class="btn-save-action btn-save" onclick="saveGame(${i})">SAVE</button>
                    ${save ? `<button class="btn-save-action btn-load" onclick="loadGame(${i})">LOAD</button>` : ''}
                    ${save ? `<button class="btn-save-action btn-delete" onclick="deleteSave(${i})">✕</button>` : ''}
                </div>
            </div>
        `;
    }
...
// Inject save modal HTML and styles into the page
(function injectSaveUI() {
    ...
    <div id="save-modal" class="modal" style="display:none;">
        <div class="modal-content save-content">
            <div class="modal-header">
                <h2 class="neon-text">SYSTEM // SAVE_MANAGER</h2>
    ...
    if (footer) {
        ...
        saveBtn.className = 'btn-main btn-save-open';
        saveBtn.innerText = '💾';
        saveBtn.title = 'Save / Load Game';
    ...
    css.textContent = `
        .save-content { width: 560px; max-width: 95vw; }
        .save-slot {
            display: flex; justify-content: space-between; align-items: center;
            background: rgba(0,0,0,0.4); border: 1px solid var(--neon-purple);
            padding: 15px 20px; margin: 12px 0;
        }
        .save-slot-num { font-size: 11px; color: var(--neon-purple); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 4px; }
        .save-slot-label { font-size: 13px; color: var(--neon-blue); font-weight: bold; }
        .save-slot-date { font-size: 10px; color: rgba(207,233,255,0.5); margin-top: 3px; }
        .save-slot-empty { font-size: 12px; color: rgba(207,233,255,0.25); font-style: italic; }
        .save-slot-actions { display: flex; gap: 8px; }
        .btn-save-action {
            padding: 8px 14px; cursor: pointer; font-family: var(--mono);
            font-size: 11px; font-weight: bold; transition: 0.2s; border: 1px solid;
        }
        .btn-save { border-color: var(--neon-blue); color: var(--neon-blue); background: transparent; }
        .btn-save:hover { background: var(--neon-blue); color: #000; box-shadow: 0 0 10px var(--neon-blue); }
        .btn-load { border-color: var(--neon-green); color: var(--neon-green); background: transparent; }
        .btn-load:hover { background: var(--neon-green); color: #000; box-shadow: 0 0 10px var(--neon-green); }
        .btn-delete { border-color: var(--neon-pink); color: var(--neon-pink); background: transparent; }
        ...
        .btn-save-open {
            ...
            flex-shrink: 0; border-color: var(--neon-green); color: var(--neon-green);
        }
        .btn-save-open:hover { background: var(--neon-green); color: #000; box-shadow: 0 0 15px var(--neon-green); }
    `;
